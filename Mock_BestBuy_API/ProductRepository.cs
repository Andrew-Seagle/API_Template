...
namespace Mock_BestBuy_API
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            return _connection.QuerySingleOrDefault<Product>("SELECT * FROM bestbuy.products WHERE ProductID = @id;", 
                new { id = id });
        }
        ...
        {
            _connection.Execute("DELETE from bestbuy.products WHERE ProductID = @id", 
                new { id = prod.ProductID });
            _connection.Execute("DELETE from bestbuy.reviews WHERE ProductID = @id", 
                new { id = prod.ProductID });
            _connection.Execute("DELETE from bestbuy.sales WHERE ProductID = @id", 
                new { id = prod.ProductID });
        }
    }
}
